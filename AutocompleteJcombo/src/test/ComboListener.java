...
public class ComboListener extends KeyAdapter {
	...
	public void keyReleased(KeyEvent key) {
		...
		((JTextField) cbListener.getEditor().getEditorComponent())
				.setText(text);
	...
	public Vector getFilteredList(String text) {
		...
		for (int a = 0; a < vector.size(); a++) {
			if (vector.get(a).toString().startsWith(text)) {
				...
				v.add(vector.get(a).toString());
			} else if (vector.get(a).toString()
					.startsWith(WordUtils.capitalizeFully(text))) {
				v.add(vector.get(a).toString());
			} else if (vector.get(a).toString()
					.startsWith(WordUtils.uncapitalize(text))) {
				v.add(vector.get(a).toString());
			}
		}
		return v;
